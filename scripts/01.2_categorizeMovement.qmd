...
save(pca_movement, file = here("data/calcMovement/pca_movement.Rda"))
```
...
save(pca_roost, file = here("data/calcMovement/pca_roost.Rda"))
```
...
contrib_space <- round(pca_space$rotation[,1:3]*-100, 2) %>%
  as.data.frame()
contrib_space
# Multiply by -1 to flip these around, because right now `pca_space` shows these both being negatively loaded.
space_use <- space_use %>%
  mutate(space_pc1 = pca_space$x[,1]*-1)
save(pca_space, file = here("data/calcMovement/pca_space.Rda"))

# Plots of the extremes
high <- space_use %>% filter(space_pc1 == max(space_pc1, na.rm = T))
low <- space_use %>% filter(space_pc1 == min(space_pc1, na.rm = T))
