...
## (for Cassidy only): Filter to just 1 year of data (2022)

```{r}
# joined <- read_feather(here::here("data", "dataPrep", "joined.feather"))
# year22 <- joined %>%
#   filter(lubridate::year(dateOnly) == "2022")
write_csv(year22, file = here::here("data", "dataPrep", "year22.csv"))
```
...
```{r load-hospital}
year22 <- read_csv("data/dataPrep/year22.csv")
...
nrow(year22) == nrow(removal_annotated) # true
