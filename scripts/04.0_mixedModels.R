...
allMetrics %>% filter(Nili_id %in% vultures) %>% ggplot(aes(x = Nili_id, y = wrapped_normDegree, group = interaction(situ, Nili_id), col = situ))+geom_violin()+facet_wrap(~season, scales = "free")+geom_point(aes(x = Nili_id, y = normDegree)) # individuals generally *do* have degrees higher than expected by chance.
...
summary(degree_6) # 
check_collinearity(degree_6) # these VIFs of 6.55 and 6.65 for situ*roost and situ*space are kind of borderline. One thing we could do is make two models and see if they agree, each removing one of the terms?
...
tar_load(allMetrics)
vultures <- sample(unique(allMetrics$Nili_id), 10)
allMetrics %>% filter(Nili_id %in% vultures) %>% ggplot(aes(x = Nili_id, y = wrapped_normStrength, group = interaction(situ, Nili_id), col = situ))+geom_violin()+facet_wrap(~season, scales = "free")+geom_point(aes(x = Nili_id, y = normStrength)) # individuals don't really seem to have strengths higher than expected by chance!
...
check_collinearity(strength_1) # remove situ*season now
