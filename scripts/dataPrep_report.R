...
load(here("data/dataPrep/data_cleaned.Rda"))
dc_nrow <- nrow(data_cleaned)
dc_vultures <- unique(data_cleaned$Nili_id)
dc_nvultures <- length(dc_vultures)
rm(data_cleaned)
...
load(here("data/dataPrep/data_masked_2.Rda"))
masked_nrow <- nrow(data_masked_2)
masked_vultures <- unique(data_masked_2$Nili_id)
