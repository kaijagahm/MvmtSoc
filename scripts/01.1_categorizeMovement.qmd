...
movementBehaviorScaled <- allMovementBehavior %>%
  mutate(across(-c(Nili_id, seasonUnique, birth_year, sex, homeRange, coreArea, coreAreaFidelity), # not scaling the space use params, because I'm not putting those into a PCA.
...
*Note also that this outcome may change quantitatively after we calculate the home ranges by better means (AKDE), but I don't expect it to be qualitatively different. AKDE calculations are currently in progress.*
...
#| message: false
#| warning: false
cor <- round(cor(red$roost_div, red$space_use_log), 2)
red %>%
  ...
  ggplot2::annotate(geom = "text", x = 4, y = 11, label = paste0("Correlation = ", cor))
```
...
#| message: false
#| warning: false
cor <- round(cor(red$space_use_log, red$movement), 2)
red %>%
  ...
  ggplot2::annotate(geom = "text", x = 14, y = -4.5, label = paste0("Correlation = ", cor))
```
