...
removed_northern <- map2(.x = removed_lfr, 
                         .y = southern_indivs, ~.x 
                         %>% filter(Nili_id %in% .y))
...
roosts <- furrr::future_map(removed_northern, ~vultureUtils::get_roosts_df(df = .x, id = "Nili_id"), .progress = T)
roosts <- roosts %>%
  furrr::future_map(., ~st_as_sf(.x, crs = "WGS84", 
...
#| eval: false
# Remove nighttime points -------------------------------------------------
removed_nighttime <- map(removed_northern, ~{
