...
library(here)
library(furrr)
future::plan(future::multisession, workers = 10) # this will last for the whole R session.
```
...
length(unique(ornitela$trackId))

# Also get some to test GPS jamming (for Ryan 2/28/24)
mindate_jamming <- "2023-10-01"
maxdate_jamming <- "2024-02-01"
test_jamming_ornitela <- vultureUtils::downloadVultures(loginObject = MB.LoginObject, removeDup = T, dfConvert = T, quiet = T, dateTimeStartUTC = mindate_jamming, dateTimeEndUTC = maxdate_jamming)
write_csv(test_jamming_ornitela, file = here("data/dataPrep/test_jamming_ornitela.csv"))
...
# Add the Nili_id
ww <- read_excel(here::here("data", "raw", "whoswho_vultures_20230920_new.xlsx"), sheet = "all gps tags")[,1:35] %>%
...
joined <- joined %>%
  mutate(Nili_id = case_when(is.na(Nili_id) ~ trackId,
                             TRUE ~ Nili_id))
length(unique(joined$Nili_id))
write_feather(joined, here::here("data", "dataPrep", "joined.feather"))
...
-   `data/raw/whoswho_vultures_20230920_new.xlsx`
...
```{r load-hospital}
joined <- read_feather(here::here("data", "dataPrep", "joined.feather"))
...
write_feather(removed_periods, here::here("data", "dataPrep", "removed_periods.feather"))
...
```{r}
removed_periods <- read_feather(here::here("data", "dataPrep", "removed_periods.feather"))
```
...
save(roosts0, file = here::here("data", "dataPrep", "roosts0.Rda"))
...
write_feather(removed_captures, here::here("data", "dataPrep", "removed_captures.feather"))
...
```{r attach-age-sex}
#| message: false
removed_captures <- read_feather(here::here("data", "dataPrep", "removed_captures.feather"))
age_sex <- read_excel(here::here("data", "raw", "whoswho_vultures_20230920_new.xlsx"), sheet = "all gps tags")[,1:35] %>%
...
length(unique(with_age_sex$Nili_id)) #201

write_feather(with_age_sex, here::here("data", "dataPrep", "with_age_sex.feather"))
...
```{r}
mask <- sf::st_read(here::here("data", "raw", "CutOffRegion.kml"))
with_age_sex <- read_feather(here::here("data", "dataPrep", "with_age_sex.feather"))
```
...
cleaned <- cleaned %>%
  mutate(timestampIsrael = lubridate::with_tz(timestamp, tzone = "Israel"),
         ...
         year = lubridate::year(dateOnly)) # XXX consider removing the downsample argument. Also this function is insanely slow and needs to be fixed, good grief.
...
save(cleaned, file = here::here("data", "dataPrep", "cleaned.Rda"))
...
```{r}
load(here::here("data", "dataPrep", "cleaned.Rda"))
```
...
```{r}
load(here::here("data", "dataPrep", "seasons_list.Rda"))
...
removed_northern <- map2(.x = seasons_list, 
                         .y = southern_indivs, ~.x 
                         %>% filter(Nili_id %in% .y))
save(removed_northern, file = here::here("data", "dataPrep", "removed_northern.Rda"))
...
```{r}
removed_lfr <- map2(removed_northern, low_fix_rate_indivs,                     ~.x %>% filter(!(Nili_id %in% .y))) 
...
subsample <- function(df, idCol = "Nili_id", 
  ...
  sub <- df %>%
    mutate(d = lubridate::date(.data[[timestampCol]])) %>% 
    group_by(.data[[idCol]]) %>%
    arrange(timestampCol, .by_group = T) %>%
    mutate(tc = cut(.data[[timestampCol]], 
                    breaks = paste(as.character(mins), 
                                   "min", sep = " "))) %>%
    ungroup() %>%
    group_by(.data[[idCol]], d, tc) %>%
    slice(1) %>%
    ungroup() %>%
    dplyr::select(-c("d", "tc"))
  return(sub)
...
downsampled_10min_forSocial <- map(removed_lfr, ~subsample(df = .x, idCol = "Nili_id", timestampCol = "timestamp", mins = 10), .progress = T)
...
map_dbl(removed_lfr, nrow) %>% setNames(season_names)
map_dbl(downsampled_10min_forSocial, nrow) %>% setNames(season_names)
...
-   `data/dataPrep/downsampled_10min_forSocial.Rda`
...
ppd <- map_dfr(removed_nighttime, ~.x %>%
                 ...
                 summarize(n = n(),
                           minBatt = min(battery_charge_percent, na.rm = T)), .progress = T)
...
# defined the "subsample" function previously above
