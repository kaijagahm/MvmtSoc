...
```{r remove-periods}
removed_periods <- vultureUtils::removeInvalidPeriods(dataset = joined, periodsToRemove = periods_to_remove)
...
data_cleaned <- vultureUtils::cleanData(dataset = removed_periods,
                     precise = F,
...
rm(joined)
gc()
save(data_cleaned, file = here("data/dataPrep/data_cleaned.Rda"))

(nrow(removed_periods)-nrow(data_cleaned))/nrow(removed_periods) # removed less than 1% of the points.
```
...
removed_captures <- removeCaptures(data = data_cleaned, 
                                       captureSites = capture_sites, 
                                       AllCarmelDates = carmel, 
                                       distance = 500, idCol = "Nili_id")

(nrow(removed_captures)-nrow(data_cleaned))/nrow(data_cleaned) # did not remove any data. Should probably check on this...
...
# Region masking
data_masked <- vultureUtils::inMaskFilter(dataset = with_age_sex, mask = mask, inMaskThreshold = 0.33, crs = "WGS84") # this takes a long time!!
save(data_masked, file = here("data/dataPrep/data_masked.Rda")) # note that this saves a 3-element list. Need to get Ryan to fix this.#XXX
load(here("data/dataPrep/data_masked.Rda"))
length(data_masked$Nili_id)
data_masked_2 <- data_masked$dataset

# Fix time zone so dates make sense ---------------------------------------
## Overwrite the dateOnly column from the new times
data_masked_2 <- data_masked_2 %>%
...
save(data_masked_2, file = here("data/dataPrep/data_masked_2.Rda")) # just the df #XXX fix
...
```{r split-seasons}
load(here("data/dataPrep/data_masked_2.Rda")) # XXX fix
