...
knitr::opts_chunk$set(include = FALSE, echo = FALSE, message = FALSE, cache = FALSE)
```

This is a condensed and simplified data preparation report. It excludes some exploratory plotting and analyses. There are records of those in 00_dataPrep.R.
...
When we download the data, there are `r (a1 <- nrow(dat))` GPS fixes from the Ornitela dataset and `r (a2 <- nrow(inpadf_touse))` fixes from the INPA dataset. Their date ranges are from `r (a3 <- min(lubridate::date(dat$timestamp), na.rm = T))` to `r (a4 <- max(lubridate::date(dat$timestamp), na.rm = T))` (Ornitela) and `r (a5 <- min(lubridate::date(inpadf_touse$timestamp), na.rm = T))` to `r (a6 <- max(lubridate::date(inpadf_touse$timestamp), na.rm = T))` (INPA). The Ornitela data contains `r (a7 <- length(unique(dat$trackId)))` individuals, and the INPA dataset contains `r (a8 <- length(unique(inpadf_touse$trackId)))`, for a total of `r (a9 <- length(unique(c(dat$trackId, inpadf_touse$trackId))))` unique birds.

Products: **dat** (ornitela data); **inpadf_touse** (inpa data, prepared to join)
Metrics: how many individuals in each, how many rows in each

## 2. Join INPA to Ornitela data. Clean ID's.
```{r}
...
After joining the two datasets together and cleaning up the names, we end up with a single dataset with `r (b1 <- length(unique(dat3$Nili_id)))` individuals and `r (b2 <- nrow(dat3))` fixes, and with a date range from `r (b3 <- min(lubridate::date(dat3$timestamp), na.rm = T))` to `r (b4 <- max(lubridate::date(dat3$timestamp), na.rm = T))`.
Product: **dat3**
Metrics: number of rows; number of individuals; date range of individuals

## 3. Remove periods in hospital/cages
```{r}
...
After removing periods when individuals were in the hospital, the number of fixes has been reduced to `r (c1 <- nrow(datAnnot))`, which is a `r `(round(100*(b2-c1)/b2, 3))`% decrease. The number of individuals has not changed: it still has `r (c2 <- length(unique(datAnnot$Nili_id)))` unique vultures.

Then we remove periods of time when individuals were in the capture cages. We are left with `r (c3 <- nrow(datAnnot_noCaptures))` fixes (a further `r `(round(100*(c1-c3)/c1, 3))`% decrease) and `r (c4 <- length(unique(datAnnot_noCaptures$Nili_id)))` individuals.

Product: **datAnnot** (after removing periodsToRemove/hospital stays), datAnnot_noCaptures (after removing capture cage periods)
Metrics: number of rows

## 4. Attach age and sex
```{r}
# Age and sex info --------------------------------------------------------
...
After attaching the age and sex information, the number of rows and individuals has not changed (`r (d1 <- nrow(datAnnot2))` rows, `r (d2 <- length(unique(datAnnot2$Nili_id)))` individuals).

Product: **datAnnot2** (then it gets renamed to datAnnot so the rest of the code will run, but datAnnot2 is a direct parallel of the state of datAnnot at the end of this).
Metrics: number of columns before and after; number of rows (to show that none were added)

## 5. Clean data (remove invalid points; keep individuals with at least 30 days total in the entire dataset)
```{r}
# Clean the data
...
Next, we clean the data with the `vultureUtils::cleanData()` function, removing invalid points. We retain only vultures that are tracked for at least 30 days in the entire dataset. We are left with `r (e1 <- nrow(datAnnotCleaned))` fixes from `r (e2 <- length(unique(datAnnotCleaned$Nili_id)))` unique vultures. This is a `r round(100*((d1-e1)/d1), 3)`% decrease in fixes and a `r (round(100*((d2-e2)/d2), 3))`% decrease in individuals.

Product: **datAnnotCleaned**
Metrics: number of individuals, number of rows

## 6. Split into seasons
```{r}
...
We've split the data into seasons now. The season names are: `r (f1 <- seasonNames_orig)`. We have a total of `r (f2 <- length(f1))` seasons. Here are the date ranges, numbers of fixes, and numbers of individuals for each season:
```{r echo = F, include = T}
mins <- map_chr(seasons_orig, ~as.character(min(lubridate::date(.x$timestamp), na.rm = T)))
maxes <- map_chr(seasons_orig, ~as.character(max(lubridate::date(.x$timestamp), na.rm = T)))
fixes <- map_dbl(seasons_orig, nrow)
inds <- map_dbl(seasons_orig, ~length(unique(.x$Nili_id)))
f3 <- data.frame(season = seasonNames_orig, startDate = mins, endDate = maxes, nFixes = fixes, nVultures = inds)
f3
```

Products: seasons_orig, seasonNames_orig
Metrics: number of seasons; number of individuals and rows per season; which seasons are included; date ranges for each season.

## 7. Remove individuals with too low a fix rate: Exclude individuals that never have a daily mode of 10 minutes
```{r}
...
```
Product: seasons, seasons_mode10
Metrics: number of individuals per season, number of fixes per season

After removing individuals that never have a 10-minute fix rate, here are the numbers of fixes and numbers of individuals for each season:
```{r echo = F, include = T}
fixes <- map_dbl(seasons_mode10, nrow)
inds <- map_dbl(seasons_mode10, ~length(unique(.x$Nili_id)))
g1 <- data.frame(season = seasonNames_orig, nFixes = fixes, nVultures = inds)
g1
```

At this point, we save the data for social analysis, saving both seasons_forSoc (which contains all individuals, regardless of fix rate) and seasons_forSoc_mode10 (which contains only individuals with a 10-minute fix rate).
...
Here are the numbers of individuals for each of these products:
```{r include = T, echo = F}
inds_reg <- map_dbl(seasons_forSoc, ~length(unique(.x$Nili_id)))
inds_mode10 <- map_dbl(seasons_forSoc_mode10, ~length(unique(.x$Nili_id)))
h1 <- data.frame(season = seasonNames_orig, allIndivs = inds_reg, highFixRate = inds_mode10) %>% mutate(propHighFixRate = round(highFixRate/allIndivs, 2))
h1
```

Products: seasons_forSoc, seasons_forSoc_mode10. These shouldn't be any different from seasons and seasons_mode10, there's just some renaming that happens.
...
```
Products: roosts_seasons and roosts_seasons_mode10
Metrics: number of individuals per season, number of rows per season

Now, we use the full dataset (not restricted to high fix rate individuals) to obtain a locations for each individual on each night. This gives us:
```{r echo = F, include = T}
cat("All individuals:\n")
(i1 <- data.frame(season = seasonNames_orig, 
                 nIndivs = map_dbl(roosts_seasons, ~length(unique(.x$Nili_id))),
                 vultureNights = map_dbl(roosts_seasons, nrow)))
```

And for comparison, the vultures and vultureNights for roosts including only the high fix rate individuals:

```{r echo = F, include = T}
cat("\nHigh fix rate individuals:\n")
(i2 <- data.frame(season = seasonNames_orig, 
                 nIndivs = map_dbl(roosts_seasons_mode10, ~length(unique(.x$Nili_id))),
                 vultureNights = map_dbl(roosts_seasons_mode10, nrow)))
```

## 10. Remove night points
```{r}
# Remove nighttime points -------------------------------------------------
before <- map_dbl(seasons, nrow)
beforeindivs <- map_dbl(seasons, ~length(unique(.x$Nili_id)))
seasons <- map(seasons, ~{
...
# changeDF
```
Product: seasons/"after" (maybe need a unique name here?)
Metrics: number of individuals per season (should be the same), number of rows per season

After removing the night points, the number of individuals per season should not have changed, but some rows have been removed.
```{r}
j1 <- data.frame(season = seasonNames_orig, 
                 nIndivsBefore = beforeindivs,
                 nIndivsNow = map_dbl(seasons, ~length(unique(.x$Nili_id))),
                 nIndivs = map_dbl(roosts_seasons, ~length(unique(.x$Nili_id))),
                 vultureNights = map_dbl(roosts_seasons, nrow))
```


<!-- ## 11. Restrict to southern individuals -->
<!-- ```{r} -->
...
<!-- rm(seasonsSF) -->
<!-- ``` -->
<!-- Product: seasons/after (unique name?) -->
<!-- Metrics: number of individuals per season, number of rows per season, boxplot of days per individual per season (so we can compare?) -->

<!-- ## 12. Include only days with > 10ppd unless the battery never dropped below 50% on that day -->
<!-- ```{r} -->
...
<!--                  ungroup()) -->
<!-- ``` -->
<!-- Product: seasons (unique name?) -->
<!-- Metrics: number of individuals per season (should not have changed since previous), number of rows per season, boxplot of days per individual per season (compare to previous) -->

<!-- ## 13. For each season, exclude individuals with < 30 valid days (after removing days with too few points) -->
<!-- ```{r} -->
...
<!-- ``` -->
<!-- Product: seasons/after (unique name) -->
<!-- Metric: number of individuals per season (should have decreased), number of points per season, boxplot of days per individual per season -->

<!-- ## 14. Get elevation information (and remove summer 2020 because it has too little data) -->
<!-- ```{r} -->
...
<!-- ``` -->
<!-- Product: seasons/after -->
<!-- Metrics: number of seasons (should be one fewer than before), number of rows per season (expect no change), number of columns, histograms of elevation by season (?) -->

<!-- ## 15. Fix rate again (why?) -->
<!-- ```{r} -->
...
<!-- ``` -->
<!-- Products: seasons, seasons_mode10 -->
<!-- Metrics: Number of individuals per season (I don't think this should have changed from the step before? I'm not clear on why we are doing this again) -->

<!-- ## 16. Downsample to 10 minutes to deal with the more frequent fixes -->
<!-- ```{r} -->
