...
The resulting data has dropped from **`r nrow(datAnnot_noCaptures)`** to
**`r nrow(datAnnot)`** rows, a
**`r round((nrow(datAnnot_noCaptures)-nrow(datAnnot))*100/nrow(datAnnot_noCaptures), 3)`% decrease**.
It has `r ncol(datAnnot)` columns and
**`r length(unique(datAnnot$Nili_id))`** unique individuals.
...
# datAnnotCleaned <- vultureUtils::cleanData(dataset = datAnnot, mask = mask, inMaskThreshold = 0.33, removeVars = F, idCol = "Nili_id", downsample = F, reMask = T) %>% st_drop_geometry()
...
length(unique(datAnnotCleaned$Nili_id))
```
...
season and decided that 3550000 latitude (in ITM coordinates) would be a
