...
For each focal individual, we characterized a pattern-based spatial phenotype (Webber et al. 2023) by calculating a variety of movement and space use measures over the course of each of the eight seasons. We used several measures to capture an individual's space use, roosting behavior, and flight movements.

### Space use

We calculated seasonal home ranges and core areas for each focal vulture, using autocorrelated kernel density estimates (AKDEs) via the {ctmm} R package [@fleming2023]. We defined the home range as the 95% AKDE and the core area as the 50% AKDE. To measure differences in how individuals used their home ranges, we calculated the ratio of the seasonal core area size to the seasonal home range size. We defined a general "space use" variable as the first principal component of a PCA containing the home range size and the core area/home range ratio. The "space use" principal component explained \[PERCENT\] of the variation in the data, with individuals low on the principal component having \[CHARACTERISTICS\] and individuals high on the principal component having \[CHARACTERISTICS\].
