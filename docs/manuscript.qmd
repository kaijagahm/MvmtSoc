...
```{r}
#| include: false
load(here("data/dataPrep/dataPrep_report_stats.Rda"))
dp <- dataPrep_report_stats
cl <- which(dp$dataset == "data_cleaned")
pctremovedcleaned <- round(100*(dp$nrow[[cl-1]] - dp$nrow[[cl]])/(dp$nrow[[cl-1]]), 2)
```
...
```{r}
#| include: false
soc <- which(dp$dataset == "downsampled_10min_forSocial")
vulturessoc <- unique(unlist(dp$vultures[[soc]]))
nvulturessoc <- length(vulturessoc)
nvulturessoc_szns <- dp$nvultures[[soc]]
```

After this cleaning, the data used for social network analysis contained `r nvulturessoc` unique vultures in total (range: `r min(nvulturessoc_szns)`-`r max(nvulturessoc_szns)`; mean = `r round(mean(nvulturessoc_szns), 2)` vultures per season, sd = `r round(sd(nvulturessoc_szns), 2)` vultures per season).
...
```{r}
#| include: false
load(here("data/calcMovement/pca_space.Rda"))
propvar_space <- as.data.frame(summary(pca_space)$importance)$PC1[2]

load(here("data/calcMovement/pca_roost.Rda"))
propvar_roost <- as.data.frame(summary(pca_roost)$importance)$PC1[2]

load(here("data/calcMovement/pca_movement.Rda"))
propvar_movement <- as.data.frame(summary(pca_movement)$importance)$PC1[2]
```
...
# NOT SHOW
plot_d_eff_space # null--no overall relationship between space use and degree
```
...
There was a significant relationship between how vultures used space and how many other individuals they encountered in flight. Vultures that consistently ranged widely encountered more others in flight, while those that concentrated their movement in smaller home ranges and smaller proportional core areas flew with fewer others. There was a slight, albeit non-statistically-significant, trend toward lower degree in the co-feeding and co-roosting networks for vultures with larger ranges.
...
# NOT SHOW
plot_d_eff_roost # null--no overall relationship between roost diversification and degree
```
...
plot_d_emt_situ_movement # going to show the plot by season instead of this one
```

```{r}
#| label: fig-d-movement-situ-season
#| fig-cap: "Relationship between normalized degree and movement, for each season and social situation."
plot_d_emt_situ_movement_season
```

Movement behavior affected degree in all three social situations. Vultures with a higher movement score co-roosted with fewer conspecifics only in the transient, pre-breeding season; otherwise movement did not affect their co-roosting degree. In the breeding and post-breeding seasons, vultures that moved more encountered more conspecifics in flight and while feeding, but this relationship did not hold in the pre-breeding season.
...
plot_s_eff_movement # null
```

There was no relationship between movement and strength, either overall or in any season or situation.
...
However, individuals with more diversified roosts had lower strength (i.e. they interacted less) overall, considering seasons and situations together.
...
Vultures with greater space use had significantly more co-flight interactions (higher strength in the co-flight network). They also tended to have a higher co-roosting strength, though the trend was not statistically significant. There was no relationship between seasonal space use and co-feeding strength.
