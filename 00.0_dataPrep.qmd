...
-   `data/derived/toRemove_lfr.Rda`

-   `data/derived/seasons_list_removed_lfr.Rda`
...
toRemove_lfr <- map(with_modes, ~.x %>%
                        ...
                        pull(Nili_id) %>%
                        unique())
save(toRemove_lfr, file = "data/derived/toRemove_lfr.Rda")
seasons_list_removed_lfr <- map2(seasons_list, toRemove_lfr, ~.x %>% filter(!(Nili_id %in% .y)))
save(seasons_list_removed_lfr, file = "data/derived/seasons_list_removed_lfr.Rda")

# Tidy up
rm(seasons_list)
rm(seasons_list_removed_lfr)
rm(with_modes)
rm(toRemove_lfr)
gc()
```
...
-   `data/derived/seasons_list_removed_lfr`

*Outputs:*

-   \`data/derived/roosts_seasons_list.

```{r}
load("data/derived/seasons_list_removed_lfr.Rda")
```

```{r}
#| eval: false
# Get roosts for each season ----------------------------------------------
roosts <- purrr::map(seasons_list_removed_lfr, ~vultureUtils::get_roosts_df(df = .x, id = "Nili_id"))
roosts <- roosts %>%
