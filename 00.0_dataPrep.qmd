...
dat3 <- dat3 %>%
  mutate(Nili_id = case_when(is.na(Nili_id) ~ trackId,
                             TRUE ~ Nili_id))
unique(dat3$Nili_id)
write_feather(dat3, "data/derived/dat3.feather")
```
...
```{r load-hospital}
dat3 <- read_feather("data/derived/dat3.feather")
...
write_feather(datAnnot_noCaptures, "data/derived/datAnnot_noCaptures.feather")
```
...
-   `data/derived/datAnnot_noCaptures.feather`
...
```{r}
datAnnot_noCaptures <- read_feather("data/derived/datAnnot_noCaptures.feather")
```
...
-   `data/derived/datAnnotCleaned.Rda`
