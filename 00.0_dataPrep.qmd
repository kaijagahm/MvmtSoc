...
write_feather(inpa, "data/derived/inpa.feather")
...
write_feather(ornitela, "data/derived/ornitela.feather")
...
-   `data/derived/joined.feather`

```{r load-inpa-ornitela}
inpa <- read_feather("data/derived/inpa.feather")
ornitela <- read_feather("data/derived/ornitela.feather")
```
...
length(unique(joined0$trackId)) # 206
...
ww %>% filter(Movebank_id %in% joined$trackId) %>% dplyr::select(Nili_id, Movebank_id) %>% distinct() %>% group_by(Nili_id) %>% summarize(n = length(unique(Movebank_id))) %>% arrange(desc(n)) # Note that there is one Nili_id in our dataset that has two associated trackId's--it's yomtov.
...
joined %>% filter(is.na(Nili_id)) %>% pull(trackId) %>% unique() # as expecfted.
joined <- joined %>%
  mutate(Nili_id = case_when(is.na(Nili_id) ~ trackId,
                             TRUE ~ Nili_id))
unique(joined$Nili_id)
write_feather(joined, "data/derived/joined.feather")
```
...
```{r load-hospital}
joined <- read_feather("data/derived/joined.feather")
...
removed <- removal_annotated %>%
  filter(status == "valid")
nrow(removed)
length(unique(removal_annotated$Nili_id))
nrow(joined)
nrow(removal_annotated)
```
...
roosts <- get_roosts_df(df = removed, id = "Nili_id", timestamp = "timestamp", x = "location_long", y = "location_lat", ground_speed = "ground_speed", speed_units = "m/s", quiet = F)
...
nrow(removed) == nrow(removed_captures) # should be TRUE. NOW we can filter.
...
length(unique(removed$Nili_id)) == length(unique(removed_captures$Nili_id)) # no change in number of individuals! As expected.

removed_captures <- removed_captures %>%
  dplyr::select(-c("ClosestCaptureSite", "ClosestCaptureDist", "Captured"))

write_feather(removed_captures, "data/derived/removed_captures.feather")
```
...
-   `data/derived/removed_captures.feather`
...
```{r attach-age-sex}
removed_captures <- read_feather("data/derived/removed_captures.feather")
```
...
# Whenever I do left_joins, I save the result as something different before reassigning it to the variable name so I can make sure the join worked before committing to it. Hence the temporary "test" object.
test <- removed_captures %>%
  dplyr::select(-c("sex")) %>%
  left_join(as, by = "Nili_id")
nrow(test) == nrow(removed_captures)
length(unique(removed_captures$Nili_id)) # 196
removed_captures <- test
rm(test) # don't need this anymore after we know the join worked.
```
...
cleaned <- vultureUtils::cleanData(dataset = removed_captures, mask = mask, inMaskThreshold = 30, removeVars = F, idCol = "Nili_id", downsample = F, reMask = T) # XXX using 30 days overall. If we want 30 days per season, will have to do that differently (e.g. split the seasons earlier)--but I think that that'll be covered later in the data cleaning anyway.
...
save(cleaned, file = "data/derived/cleaned.Rda")
load("data/derived/cleaned.Rda")
```
...
-   `data/derived/seasons_orig.Rda`

-   `data/derived/seasonNames_orig.Rda`
...
## Remove individuals with low fix rate

Find the mode (most common) observation frequency for each individual on each day. Then exclude individuals who never have observations at 10-minute frequencies.

We keep the ones that theoretically have a 10-minute fix rate, even if in practice they often have much less frequent observations.
...
-   `data/derived/toKeep_fixRate.Rda`
...
## Save dataset for social analysis
...
-   `data/derived/seasons_forSoc.Rda`

-   `data/derived/seasons_forSoc_mode10.Rda`
...
-   `data/derived/roosts_seasons.Rda`

-   `data/derived/roosts_seasons_mode10.Rda`
...
XXX do we not also remove nighttime points for seasons_mode10?
...
XXX why are we using `seasons_orig` here? shouldn't it be `seasons`?
...
-   `data/derived/seasonsSF.Rda`

-   `seasons` XXX seems wrong!
...
## Remove individuals with too few points per day
...
## Remove individuals with too few days tracked
...
-   `data/derived/elevs_z10_seasons.Rda`

-   `data/derived/groundElev_z10.Rda`
...
## Restrict to mode10 individuals
...
-   `data/derived/seasons.Rda`

-   `data/derived/seasons_mode10.Rda`
...
Downsampling data to 10-minute intervals exclusively
...
-   `data/derived/seasons_10min.Rda`

-   `data/derived/seasons_mode10_10min.Rda`

XXX this is confusing. Need to rename after concepts, not minutes.
