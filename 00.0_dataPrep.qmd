...
I suppose we could leave in the low fix rate individuals, especially for co-roosting. \[Actually, maybe we should leave them in for co-roosting, but not the others... which makes me think maybe we should leave them in for roost and social, just not movement, because they can contribute to the background information. Aaaarrrrrrggghhhhh.... XXX\]
...
roosts <- purrr::map(seasons_list, ~vultureUtils::get_roosts_df(df = .x, id = "Nili_id"))
roosts <- roosts %>%
...
```{r}
removed_lfr <- map2(seasons_list, low_fix_rate_indivs,
