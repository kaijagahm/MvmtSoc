...
-   `data/derived/removed_periods.feather`
...
write_feather(removed_periods, "data/derived/removed_periods.feather")
...
-   `data/derived/roosts0.Rda`

-   `data/derived/removed_captures.feather`

```{r}
removed_periods <- read_feather("data/derived/removed_periods.feather")
```
...
-   `data/derived/with_age_sex.feather`
...
write_feather(with_age_sex, "data/derived/with_age_sex.feather")
...
-   `data/derived/cleaned.feather`

```{r}
mask <- sf::st_read("data/raw/CutOffRegion.kml")
with_age_sex <- read_feather("data/derived/with_age_sex.feather")
```
...
-   `data/derived/seasons_list.Rda`

-   `data/derived/season_names.Rda`
...
save(seasons_list, file = "data/derived/seasons_list.Rda")
save(season_names, file = "data/derived/season_names.Rda")
...
-   `data/derived/removed_lfr.Rda`

```{r}
load("data/derived/seasons_list.Rda")
```
...
removed_lfr <- map2(seasons_list, toRemove_lfr, ~.x %>% 
                                   filter(!(Nili_id %in% .y)))

save(removed_lfr, 
     file = "data/derived/removed_lfr.Rda")
...
We are going to need to get nightly roost locations for all of the remaining vultures. We have to do this now because getting roost locations requires using points that occur at night, which we will eventually want to remove in order to do the movement analysis.

*Inputs:*

-   `data/derived/removed_lfr`
...
```{r}
load("data/derived/removed_lfr.Rda")
```
...
# Further processing for movement data
...
-   `data/derived/removed_nightime.Rda`

```{r}
load("data/derived/removed_lfr.Rda")
load("data/derived/season_names.Rda")
```
...
save(removed_nighttime, file = "data/derived/removed_nighttime.Rda")
...
-   `data/derived/removed_nighttime.Rda`
-   `data/derived/season_names.Rda`

*Outputs:*

-   `data/derived/removed_northern.Rda`

```{r}
load("data/derived/removed_nighttime.Rda")
load("data/derived/season_names.Rda")
```
...
save(removed_northern, file = "data/derived/removed_northern.Rda")
...
-   `data/derived/removed_lowppd.Rda`

```{r}
load("data/derived/removed_northern.Rda")
```
...
save(removed_lowppd, file = "data/derived/removed_lowppd.Rda")
...
-   `data/derived/removed_too_few_days.Rda`
-   `data/derived/season_durations.Rda`

```{r}
load("data/derived/removed_lowppd.Rda")
load("data/derived/season_names.Rda")
```
...
save(removed_too_few_days, file = "data/derived/removed_too_few_days.Rda")
save(season_durations, file = "data/derived/season_durations.Rda")
...
-   `data/derived/removed_too_few_days`

*Outputs:*

-   `data/derived/elevs_z10.Rda`

-   `data/derived/with_altitudes.Rda`

```{r}
load("data/derived/removed_too_few_days.Rda")
```
...
save(elevs_z10, file = "data/derived/elevs_z10.Rda")
```

```{r}
load("data/derived/elevs_z10.Rda")
```
...
save(with_altitudes, file = "data/derived/with_altitudes.Rda")
...
-   `data/derived/downsampled_10min.Rda`

```{r}
load("data/derived/with_altitudes.Rda")
load("data/derived/season_names.Rda")
```
...
save(downsampled_10min, file = "data/derived/downsampled_10min.Rda")
